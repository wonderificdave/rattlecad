 ...
 namespace eval cv_custom {
	...
	proc createDimension {cv_Name BB_Position type {active {on}}} {
		...
		switch $type {
			...
			cline_frame {
						$cv_Name create centerline 	[ project::flatten_nestedList $TopTube(Steerer) $TopTube_Seat_virtual ] \
			...
			geometry_bg {
						...
						set _dim_ST_YPosition	[ $cv_Name dimension  length  	[ project::flatten_nestedList  $BottomBracket(Position) $Saddle(Position) ] \
															...
															gray50 ] 
						set _dim_SD_Height		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $BaseCenter  $Saddle(Position) ] \
															vertical    [expr -660 * $stageScale]  [expr -190 * $stageScale]  \
															gray50 ] 
						set _dim_HB_Height		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $HandleBar(Position) $BaseCenter ] \
						...
						set _dim_CS_LengthX 	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$RearWheel_Ground  $BaseCenter ] \
															horizontal  [expr   70 * $stageScale]   0 \
															gray50 ] 
						set _dim_Wh_Distance	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$RearWheel_Ground  $FrontWheel_Ground ] \
															horizontal  [expr  130 * $stageScale]	0 \
															gray50 ] 
						set _dim_FW_Lag			[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$FrontWheel_Ground  $Steerer(Ground) ] \
															horizontal  [expr   70 * $stageScale]   [expr  -70 * $stageScale] \
															gray20 ] 
															
						set _dim_ST_Virtual		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $BottomBracket(Position) $TopTube_Seat_virtual ] \
															aligned     [expr   80 * $stageScale]   [expr   90 * $stageScale] \
															gray50 ] 

						set _dim_BT_Clearance	[ $cv_Name dimension  length  	[ project::flatten_nestedList  $help_91  $help_92 ] \
			...
			geometry_fg {
						...
						set _dim_FW_Distance	[ $cv_Name dimension  length  	[ project::flatten_nestedList  $BottomBracket(Position)  $FrontWheel(Position)] \
															...
															$colour(result) ] 
						set _dim_FW_DistanceX	[ $cv_Name dimension  length  	[ project::flatten_nestedList  $BaseCenter  $FrontWheel(Position) ] \
															horizontal  [expr   70 * $stageScale]   0 \
															$colour(result) ] 
						set _dim_BB_Height 		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $BottomBracket(Position)	$BaseCenter] \
															vertical    [expr  150 * $stageScale]   [expr   -20 * $stageScale]  \
															$colour(result) ]
						set _dim_TT_Virtual		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $TopTube_Seat_virtual  $TopTube(Steerer)] \
						...
						set _dim_LC_Position_x	[ $cv_Name dimension  length  	[ project::flatten_nestedList  $LegClearance(Position)  $BottomBracket(Position) ] \
															horizontal  [expr   80 * $stageScale]   0  \
															$colour(third) ] 
						set _dim_LC_Position_y	[ $cv_Name dimension  length  	[ project::flatten_nestedList  $LegClearance(Position)  $BaseCenter ] \
															vertical    [expr  -50 * $stageScale]   [expr   160 * $stageScale]  \
															$colour(third) ] 
						set _dim_CR_Length 		[ $cv_Name dimension  radius    [ project::flatten_nestedList  $BottomBracket(Position)  $help_91] \
						...
						set _dim_CS_Length 		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $RearWheel(Position)  $BottomBracket(Position)] \
															...
															$colour(secondary) ] 
						set _dim_HT_Angle  		[ $cv_Name dimension  angle   	[ project::flatten_nestedList  $Steerer(Ground)  $Steerer(Fork)  $BaseCenter ] \
															150   0  \
															$colour(secondary) ]
						set _dim_RW_Radius 		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $RearWheel(Position)  $RearWheel_Ground ] \
															vertical    [expr   130 * $stageScale]    0 \
															$colour(secondary) ] 
						set _dim_FW_Radius 		[ $cv_Name dimension  length  	[ project::flatten_nestedList  $FrontWheel(Position)  $FrontWheel_Ground ] \
			...
			frameTubing_bg {
							...
							# -- Dimensions ------------------------
							#
						set _dim_ST_Length_01	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$BottomBracket(Position)  $SeatTube(TopTube) ] \
						...
						set _dim_Down_Seat_Angle	[ $cv_Name dimension  angle  	[ project::flatten_nestedList [list $BottomBracket(Position) $DownTube(Steerer) $TopTube(SeatTube) ] ] \
															110   0 \
															gray50 ]
						set _dim_Seat_CS_Angle		[ $cv_Name dimension  angle  	[ project::flatten_nestedList [list $BottomBracket(Position) $TopTube(SeatTube) $ChainSt_SeatSt_IS] ] \
															110   0 \
															gray50 ]
			...
			summary_fg {
						...
						set _dim_CS_LengthX 	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$RearWheel_Ground  $BaseCenter ] \
																				horizontal  [expr   70 * $stageScale]   0 \
																				gray50 ] 
						set _dim_FW_DistanceX	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$BaseCenter  $FrontWheel_Ground ] \
																				horizontal  [expr   70 * $stageScale]   0 \
																				gray50 ] 
						set _dim_Wh_Distance	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$RearWheel_Ground  $FrontWheel_Ground ] \
																				horizontal  [expr  130 * $stageScale]	0 \
			...
			frameDrafting_bg {
						...
						set _dim_SS_Length			[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$RearWheel(Position)  $SeatStay(SeatTube) ] \
																					aligned  	[expr  -160 * $stageScale]	0 \
																					darkblue ]
						set _dim_SS_ST_Offset		[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$SeatTube(TopTube)  $SeatStay(SeatTube) ] \
																					aligned  	[expr    60 * $stageScale]	[expr   65 * $stageScale] \
																					darkblue ]
						if { $RearDrop(OffsetSSPerp) != 0 } {
								set _dim_SS_DO_Offset	\
													[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$SeatStay(SeatTube) $ChainSt_SeatSt_IS $RearWheel(Position)] \
																					perpendicular [expr -75 * $stageScale]	[expr  -35 * $stageScale] \
																					gray30 ]
							}
						if { $RearDrop(OffsetCSPerp) != 0 } {
								set _dim_CS_DO_Offset	\
													[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$BottomBracket(Position) $ChainSt_SeatSt_IS $RearWheel(Position)] \
																					perpendicular [expr -95 * $stageScale]	[expr   35 * $stageScale] \
																					gray30 ]
							}
						set _dim_DT_Offset			[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$HeadTube(Fork)  $DownTube(Steerer) ] \
																					aligned  	[expr    70 * $stageScale]	[expr    0 * $stageScale] \
																					darkblue ]
						set _dim_TT_Offset			[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$TopTube(Steerer)  $HeadTube(Stem) ] \
						...
						set _dim_STR_Length			[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$pt_01  $pt_02 ] \
						...
						set _dim_Seat_CS_Angle		[ $cv_Name dimension  angle  	[ project::flatten_nestedList [list $BottomBracket(Position) $TopTube(SeatTube) $ChainSt_SeatSt_IS] ] \
																					 90   0 \
																					darkred ]
						set _dim_SS_CS_Angle		[ $cv_Name dimension  angle  	[ project::flatten_nestedList [list $ChainSt_SeatSt_IS $BottomBracket(Position) $SeatStay(SeatTube)] ] \
																					 90   0 \
																					darkred ]
							set pt_01				[ vectormath::addVector	$BottomBracket(Position) {-1 0} ]
						set _dim_SeatTube_Angle		[ $cv_Name dimension  angle  	[ project::flatten_nestedList [list $BottomBracket(Position) $SeatTube(TopTube) $pt_01] ] \
						...
						if {$Rendering(BottleCage_DT_L) != {off}} {
										...
										#set dimension		[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$pt_05  $pt_03 ] \
																								aligned  	  [expr -145 * $stageScale]	0 \
																								gray50 ]
										#set dimension		[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$pt_02  $pt_03 $BB_Position ] \
						...
						set _dim_HeadSet_Bottom 	[ $cv_Name dimension  length  	[ project::flatten_nestedList [list $pt_01 $pt_02] ] \
	...
	proc createDimensionType {cv_Name BB_Position type {updateCommand {}}} {
		...
		switch $type {
			...
			DownTube_Offset {
						...
						set dimension 		[ $cv_Name dimension  length  	[ project::flatten_nestedList [list $pt_01 $pt_02 $pt_03] ] \
																				{perpendicular}    $dim_distance $dim_offset \
																				darkred ]																				
						if {$updateCommand != {}} {	$cv_Name	bind	$dimension	<Double-ButtonPress-1>  \
																		[list frame_geometry::createEdit  %x %y  $cv_Name  \
			...
			ForkHeight {
						...
						set help_fk					[ vectormath::addVector   	$Steerer(Fork) 	[ vectormath::unifyVector $Steerer(Stem)  $Steerer(Fork)   $Fork(Height) ] ]
						if {$Fork(Rake) != 0} {
							set dimension	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$help_fk $FrontWheel $Steerer(Fork)  ] \
																				perpendicular [expr  (110 - $Fork(Rake)) * $stageScale]    [expr  -80 * $stageScale] \
																				darkred ] 
						} else {
							set dimension	[ $cv_Name dimension  length  	[ project::flatten_nestedList  	$FrontWheel(Position) $Steerer(Fork)  ] \
			...
			RearWheel_Clearance {
						...
						set dimension 		[ $cv_Name dimension  length  	[ project::flatten_nestedList [list $pt_rw $pt_is] ] \
																				aligned    [expr -70 * $stageScale]  [expr 50 * $stageScale] \
																				gray50 ]																				
							#set dimension 		[ $cv_Name dimension  radius  	[ project::flatten_nestedList [list $pt_03 $pt_rw ] ] \
	...
	proc createCenterline {cv_Name BB_Position {extend_Saddle {}}} {
		...
		$cv_Name create centerline [ project::flatten_nestedList  $SeatStay(SeatTube)	$ChainSt_SeatSt_IS  ] -fill gray60 -tags __CenterLine__ 
		$cv_Name create centerline [ project::flatten_nestedList  $ChainSt_SeatSt_IS	$BottomBracket(Position)		] -fill gray60 -tags __CenterLine__
		if {$extend_Saddle == {}} {
