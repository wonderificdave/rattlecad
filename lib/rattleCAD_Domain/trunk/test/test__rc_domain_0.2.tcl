...
namespace eval test__rc_domain {
    ...
    lappend auto_path           [file join $BASE_Dir ..]
        ...
        # -- Packages  ---------------
    package require   _rc_domain  0.2
    ...
    set sampleDOM   [$sampleDOC documentElement]
        ...
        # -- init bikeGeomtry  ------
    _rc_domain::set_newProject $sampleDOM
    
    
        # -- get result as XML  -----
    set runTime_XML [_rc_domain::get_projectXML]
        # puts [$runTime_XML asXML]
    
    
        # -- get result as DICT  ----
    set runTime_DICT [_rc_domain::get_projectDICT]       
    appUtil::pdict $runTime_DICT 0 "    "
    
    
        # -- update Values  ----
    puts "\n  ... _rc_domain::get_Value: Personal/HandleBar_Distance"
        set persKey     Personal/HandleBar_Distance     
        set posKey      Result/Position/HandleBar     
        set persValue   [_rc_domain::get_Value $persKey    value]       
    puts "       -> \$persValue   $persValue"
        set resPosition [_rc_domain::get_Value $posKey     position] 
    puts "       -> \$resPosition $resPosition"
        
        set persValue   [_rc_domain::set_Value $persKey    [expr $persValue + 5]] 
    puts "       -> \$persValue   $persValue"
        set resPosition [_rc_domain::get_Value $posKey     position] 
...
puts "     ... root_ProjectDOM      [file join $::APPL_Config(CONFIG_Dir) $projectFile]"
    ...
    # expired
    # set dictInit    [_rc_domain::init_Prereq  $::APPL_Config(root_InitDOM)]
