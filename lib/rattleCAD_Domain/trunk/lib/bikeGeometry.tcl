 ...
 namespace eval bikeGeometry {
        ...
        # --------------------------------------------
            # export procedures to parent namespace
        
        namespace export set_newProject  
        namespace export set_forkConfig  
        
        namespace export get_projectXML  
    ...
    proc set_newProject {_projectDOM} {
            ...
            # --- report Fork Settings ------------------
                #                  
            set _forkDOM   [$rc_Domain::project::initDOM selectNode Fork/_bikeGeometry_default_]  
            if {$_forkDOM != {}} {
                puts ""
                puts "        -- <W> ----------------------------------------------------"
                puts "           <W> bikeGeometry:"
                puts "           <W>       ...  using default Fork Settings"
                puts "           <W>         see -> \$rc_Domain::bikeGeometry::initDOM /root/Fork"
                puts "           <W>"
                puts "" 
            }      
            ...
            # --- set the Geometry DOM Object -----------
                #
            set rc_Domain::project::projectDOM $_projectDOM
              
                
            # --- update the project to current level ---
                #  ... and get the required post updates
                #
            set postUpdate [rc_Domain::project::update_Project]
        
        
            # --- make the current DOM Object alive ----
                #
            rc_Domain::project::dom_2_runTime
        
                  # 
                  # puts "     -- 004 --------"
                  # puts [$rc_Domain::project::projectDOM asXML]
                  # #exit
                
            # --- compute geometry ----------------------
                #
            rc_Domain::bikeGeometry::set_base_Parameters
            ...
            foreach key [dict keys $postUpdate] {
                    ...
                    foreach valueKey [dict keys $valueDict] {
                        ...
                        rc_Domain::bikeGeometry::set_Value $key/$valueKey $newValue update
                    }
                        # rc_Domain::project::pdict $valueDict
            }
    ...
    proc set_forkConfig {_forkDOM} {
            rc_Domain::project::add_forkSetting $_forkDOM
    }
    #-------------------------------------------------------------------------
    ...
    #-------------------------------------------------------------------------
        #  get current projectDOM as DICT
    proc get_projectDICT {} {
            return [rc_Domain::project::runTime_2_dict]
    }
    #-------------------------------------------------------------------------
    ...
    proc set_Value {xpath value {mode {update}}} {
             ...
             foreach {_array _name path} [rc_Domain::project::unifyKey $xpath] break
                 # puts "     ... $_array  $_name"
             ...
             # --- exception on mode == force ---
                 #  
             if {$mode == {force}} { 
                 eval set [format "rc_Domain::project::%s(%s)" $_array $_name] $value
                 rc_Domain::bikeGeometry::set_base_Parameters
                 return $value
             }              
             ...
             switch $xpath {
                 ...
                 {Lugs/RearDropOut/Direction} {
                         set newValue    $value
                         rc_Domain::project::setValue [format "%s(%s)" $_array $_name] value $newValue
                         rc_Domain::bikeGeometry::set_base_Parameters  
                         return $newValue
                     }
    ...
    proc get_Value {xpath type args} {
            return [rc_Domain::project::getValue $xpath $type $args]
    }
    #-------------------------------------------------------------------------
    ...
    proc get_Object {object index {centerPoint {0 0}} } {
            ...
            if {$object == {DEBUG_Geometry}} {
                    ...
                    return [ rc_Domain::vectormath::addVectorPointList  $centerPoint $returnValue ]
            }
            ...
            switch -exact $index {
                ...
                polygon    {    set returnValue    {}
                            switch -exact $object {
                                ...
                                # puts "    ... $branch"
                            set svgList    [ rc_Domain::project::getValue Result($branch)    polygon ]
                            foreach xy $svgList {
                                foreach {x y} [split $xy ,] break
                ...
                position {
                            ...
                            set pointValue    [ rc_Domain::project::getValue Result($branch)    position ]    ; # puts "    ... $pointValue"
                ...
                direction {
                            ...
                            set directionValue    [ rc_Domain::project::getValue Result($branch)    direction ]    ; # puts "    ... $directionValue"
    ...
    proc set_resultParameter {_array _name value} {
            ...
            switch -glob $_name {
                ...
                {Length/BottomBracket/Height}    {
                      ...
                      set oldValue                $rc_Domain::project::Result(Length/BottomBracket/Height)
                      # 3.2.76 set oldValue       $rc_Domain::project::Temporary(BottomBracket/Height)
                      ...
                      set oldValue                $rc_Domain::project::Custom(BottomBracket/Depth)
                ...
                {Angle/HeadTube/TopTube} {
                      ...
                      set _updateValue($xpath)    $HeadTopTube_Angle
                        ...
                        # --- update value
                        #
                      set HeadTube_Angle          $rc_Domain::project::Custom(HeadTube/Angle)
                ...
                {Angle/SeatTube/Direction} {
                          ...
                          # puts ""
                      set oldValue        $rc_Domain::project::Result(Angle/SeatTube/Direction)
                      set SP_Setback      [rc_Domain::project::getValue Component(SeatPost/Setback)   value]
                      set length_Setback  [expr $SP_Setback * sin([rc_Domain::vectormath::rad $value])]
                      set height_Setback  [expr $SP_Setback * cos([rc_Domain::vectormath::rad $value])]
                        # puts "    -> value $value"
                        # puts "    -> oldValue $oldValue"
                        # puts "    -> SP_Setback $SP_Setback"
                        # puts "    -> length_Setback $length_Setback"
                        # puts "    -> height_Setback $height_Setback"
                      set ST_height       [expr [rc_Domain::project::getValue Personal(Saddle_Height)   value] - [rc_Domain::project::getValue Component(Saddle/Height)   value] + $height_Setback]
                      set length_SeatTube [expr $ST_height / tan([rc_Domain::vectormath::rad $value])]
                ...
                {Length/SeatTube/VirtualLength} {
                        ...
                        # SeatTube Offset
                        #
                      set oldValue                [rc_Domain::project::getValue [format "%s(%s)" $_array $_name] value]
                      set newValue                [set_Value $xpath  $value format]
                      set _updateValue($xpath)    $newValue
                      set delta                   [expr $newValue - $oldValue]
              
                      set offsetSeatTube          [rc_Domain::vectormath::rotateLine {0 0} $delta [expr 180 - $rc_Domain::project::Result(Angle/SeatTube/Direction)]]
                      set offsetSeatTube_x        [lindex $offsetSeatTube 0]
                        # puts "   -> $offsetSeatTube"
                  
                        # HeadTube Offset - horizontal
                        #
                      set deltaHeadTube           [expr [lindex $offsetSeatTube 1] / sin($rc_Domain::project::Custom(HeadTube/Angle) * $rc_Domain::vectormath::CONST_PI / 180) ]
                      set offsetHeadTube_x        [expr [lindex $offsetSeatTube 1] / tan($rc_Domain::project::Custom(HeadTube/Angle) * $rc_Domain::vectormath::CONST_PI / 180) ]
              
                        # HeadTube Offset - horizontal & length
                        #
                        # rc_Domain::project::remove_tracing ; #because of setting more then one parameter at once
                        #
                ...
                {Length/HeadTube/StackHeight} {
                      ...
                      set deltaHeadTube           [expr $delta / sin($rc_Domain::project::Custom(HeadTube/Angle) * $rc_Domain::vectormath::CONST_PI / 180) ]
                      set offsetHeadTube_x        [expr $delta / tan($rc_Domain::project::Custom(HeadTube/Angle) * $rc_Domain::vectormath::CONST_PI / 180) ]
                        ...
                        #
                        # rc_Domain::project::remove_tracing ; #because of setting two parameters at once
                        #
                      ...
                      set_Value                   $xpath      $newValue
                        ...
                        #
                      set xpath                   FrameTubes/HeadTube/Length
                      set oldValue                $rc_Domain::project::FrameTubes(HeadTube/Length)
                      set newValue                [expr $rc_Domain::project::FrameTubes(HeadTube/Length) + $deltaHeadTube ]
                      set_Value                   $xpath     $newValue
                    #
                      return
                }
                ...
                {Length/RearWheel/horizontal} {
                      ...
                      set _updateValue($xpath)    $newValue
                          #set delta                  [expr $newValue - $oldValue]
                      set bbDepth                 $rc_Domain::project::Custom(BottomBracket/Depth)
                ...
                {Length/FrontWheel/diagonal}    {
                    ...
                    #
                      ...
                      set help_03   [ rc_Domain::vectormath::cathetusPoint    $HandleBar(Position)    $FrontWheel(Position_tmp)    $vect_02  close ]
                      set vect_HT   [ rc_Domain::vectormath::parallel      $help_03                  $FrontWheel(Position_tmp)    $Fork(Rake) ]
                        # puts "                 <D> ... $vect_HT"
                      ...
                      set newValue                [rc_Domain::vectormath::angle    $help_01 $help_02 $help_03 ]
                ...
                {Length/Saddle/Offset_BB_ST}    {
                        # puts "               ... [format "%s(%s)" $_array $_name] $xpath"
                      set newValue                [set_Value $xpath  $value format ]
                      set height                  [rc_Domain::project::getValue [format "%s(%s)" Personal Saddle_Height] value ]
                      set angle                   [rc_Domain::vectormath::dirAngle {0 0} [list $newValue $height] ]
                ...
                {Length/Saddle/Offset_BB_Nose}    {
                      ...
                      set delta                   [expr -1.0 * ($newValue - $oldValue) ]
                        ...
                        # --- set HandleBar(Distance)
                        #
                      set newValue                [expr $rc_Domain::project::Component(Saddle/LengthNose) + $delta ]
