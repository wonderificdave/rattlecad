 ...
 namespace eval project {
    ...
    #-------------------------------------------------------------------------
        ...
        #               
    set fp [open [file join $packageHomeDir .. etc initConfig.xml] ]
    ...
    #-------------------------------------------------------------------------
        ...
        #  
    set fp [open [file join $packageHomeDir .. etc initResult.xml] ]
    fconfigure      $fp -encoding utf-8
    set resXML     [read $fp]
    close           $fp          
    set resDoc     [dom parse $resXML]
    set resultDOM  [$resDoc documentElement]
    set resultNode [$resultDOM selectNode Result]      
    
    #-------------------------------------------------------------------------
        #  fill templateDOM
        #    ... fill with default settings
        #  
    set fp [open [file join $packageHomeDir .. etc templateProject.xml] ]
    fconfigure      $fp -encoding utf-8
    set templXML    [read $fp]
    close           $fp          
    set templDoc    [dom parse $templXML]
    set templateDOM [$templDoc documentElement]
    
    #-------------------------------------------------------------------------
        #  fill projectDOM
    set projDOC     [dom parse $templXML]
    ...
    proc setValue {key type args} {
            ...
            variable templateDOM
                ...
                # -- get different representation of the key
            foreach {_array _name _path} [unifyKey $key] break

            
                # -- get the value type of key
            set myNode [$templateDOM selectNode $_path]
            if {![catch {set valueType [$myNode getAttribute {type}]} eID]} {
                puts "       -> valueType: $valueType"
            } else {
                set valueType {_undefined}
            }
            ...
            switch -exact $type {
                    ...
                    direction     {    
                                    ...
                                    set value    [ rc_Domain::vectormath::unifyVector    $p0 $p1]
                                    ...
                                    set check_name [array names [namespace current]::$_array $_name]
                                    if { $check_name != {} } {
                                        set angleDegree    [format "%.5f" [ rc_Domain::vectormath::dirAngle {0 0} $p1] ]
                                        #puts "  -- 02 --"
                                    ...
                                    if { $check_name != {} } {
                                        ...
                                        set angleRadiant    [format "%.6f" [ rc_Domain::vectormath::rad $angleDegree] ]
