	...
	# interp eval  {canvas new_command} {tk_messageBox -message "polydraw alias - rectangle"}

 
		# --- proc looks on $list for $element -> 0/1
	proc polydraw {w} {
		...
		#interp create {} canvas {} new_command
		#proc new_command {} {.d insert end "polydraw alias - rectangle"}
		#interp alias canvas new_command 
		#canvas alias new_command 
	}
  ...
  #--------------------------------------- more general routines
	...
	proc has {list element} {expr {[lsearch $list $element]>=0}}
		...
		# --- custom procedure -  create_rectangle
	proc create_rectangle {x y width height} {
			.d insert end "create_rectangle  $x - $y - $width - $height\n"	
			set c_tag [.c create rectangle $x  $y  $width  $height]
			.d insert end "   $c_tag created"
	}
	...
	if 1 {
		 ...
		 #------------------------------------------------ Application2 - Outlining:
			...
			proc ShowPoly {w} {
				...
				#: Main :
			pack [canvas .c -width 520 -height 420 -bg white] -fill both -expand 1
			pack [frame  .f ]                       -expand y -fill x
				  button .f.b1 -text " create rectangle 1 " -command {create_rectangle 30 50 180 160}
				  button .f.b2 -text " create rectangle 2 " -command {create_rectangle 90 180 190 250}
				  button .f.b3 -text " alias "              -command {.c new_command}
			pack .f.b1 .f.b2 .f.b3 -side left

			
			pack [text   .d -width  50 -height   9] -expand y -fill x
			pack [text   .t -width  50 -height   9] -expand y -fill x
			polydraw .c
			help     .t
			...
			puts "foreach p \[.c find withtag poly] \{puts \"\$p: \[.c coords \$p ]\\n\"\}"
			  ...
			  # --- update title by mouse position ---------
