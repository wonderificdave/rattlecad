 ...
 #
    ...
    proc bikeGeometry::create_GeometryFront {} {
            ...
            set Geometry(FrontWheel_Distance_y) [ expr $Geometry(BottomBracket_Depth) - $Geometry(RearWheel_Radius) + $Geometry(FrontWheel_Radius) ]
                #
            ...
            set help_06 [ lindex $vect_04 1 ]
                #
            set FrontWheel(Position)    [ vectormath::intersectPoint    $help_05  $help_06 [list 0 $Geometry(FrontWheel_Distance_y)] [list 200 $Geometry(FrontWheel_Distance_y)] ]
            set Geometry(FrontWheel_DistanceBB_x)   [ lindex $FrontWheel(Position) 0]
            set Geometry(FrontWheel_DistanceBB)     [ expr hypot($Geometry(FrontWheel_DistanceBB_x),$Geometry(FrontWheel_DistanceBB_x)) ]
                #
            set Steerer(FrontWheel)     [ vectormath::rotateLine    $FrontWheel(Position)    $Geometry(Fork_Rake)    [expr 270 - $Geometry(HeadTube_Angle)] ]
            set Steerer(Fork)           [ vectormath::addVector     $Steerer(FrontWheel)     [ vectormath::unifyVector  $Steerer(FrontWheel)  $Steerer(Stem)  $Geometry(Fork_Height) ] ]
               #
            set help_08  [ vectormath::addVector    $Geometry(BottomBracket_Ground) {200 0}]
                #
            set Steerer(Ground)         [ vectormath::intersectPoint        $Steerer(Stem) $Steerer(Fork)      $Geometry(BottomBracket_Ground)  $help_08 ]
                #
            #set Fork(DropoutPosition)   [list $Geometry(FrontWheel_DistanceBB_x)    [expr $Geometry(BottomBracket_Depth) + ($Geometry(FrontWheel_Radius) - $Geometry(RearWheel_Radius))]]
                #
            #puts "  bikeGeometry::create_GeometryFront ... $FrontWheel(Position)"
            #puts "  bikeGeometry::create_GeometryFront ... $Fork(DropoutPosition)"
                #
            return
                #
    }
    proc bikeGeometry::create_GeometryRear {} {
            ...
            set Geometry(RearWheel_Radius)          [ expr 0.5*$Geometry(RearRim_Diameter) + $Geometry(RearTyre_Height) ]
                #
            set Geometry(RearWheel_DistanceBB_x)    [ expr sqrt(pow($Geometry(ChainStay_Length),2)  - pow($Geometry(BottomBracket_Depth),2)) ]
                #
            return
                #
    }   
    proc bikeGeometry::create_GeometryCenter {} {
            ...
            set Geometry(BottomBracket_Ground)           [ list 0    [expr - $Geometry(RearWheel_Radius) + $Geometry(BottomBracket_Depth) ] ]
                #
            ...
            set Saddle(Nose)            [ vectormath::addVector  $Saddle(Position) [list [expr $Saddle(NoseLength) + $Saddle(Offset_x)] -15] ]
                #
            set SeatPost(Height)        [ expr $Geometry(Saddle_Height) - $Saddle(SaddleHeight) ]
            ...
            set LegClearance(Position)  [ list $TopTube(PivotPosition)  [expr $Geometry(Inseam_Length) - ($Geometry(RearWheel_Radius) - $Geometry(BottomBracket_Depth)) ] ]
                #
                #
            set Saddle(Proposal)        [ vectormath::rotateLine {0 0}  [ expr 0.88*$Geometry(Inseam_Length) ]  [ expr 180 - $SeatTube(Angle) ] ]
                #
            set help_08  [ vectormath::addVector    $Geometry(BottomBracket_Ground) {200 0}]
                #
            set SeatTube(Ground)    [ vectormath::intersectPoint        $SeatPost(SeatTube) $SeatTube(BottomBracket)      $Geometry(BottomBracket_Ground)  $help_08 ]
                #
            set Geometry(SeatTubeSaddle)    [ vectormath::intersectPoint [list 0 [lindex $Saddle(Position) 1] ] [list 100 [lindex $Saddle(Position) 1]] $SeatTube(BottomBracket) $SeatPost(SeatTube) ]
                #
            # set Result(Position/BottomBracketGround)     $Geometry(BottomBracket_Ground)
            ...
            set Geometry(LegClearance)      [list $TopTube(PivotPosition)     [expr $Geometry(Inseam_Length) - ($Geometry(RearWheel_Radius) - $Geometry(BottomBracket_Depth)) ]]
    ...
    proc bikeGeometry::create_SummarySize {} {
            ...
            set summaryLength [format "%.6f" [ expr $Geometry(RearWheel_DistanceBB_x) + $Geometry(FrontWheel_DistanceBB_x)]]
    ...
    proc bikeGeometry::create_RearDropout {} {
        ...
        variable RearDropout
            #
        set RearDropout(Position)   [ list [expr -1.0 * $Geometry(RearWheel_DistanceBB_x)] $Geometry(BottomBracket_Depth) ]
            #
        return
            #
    }
