 ...
 #
    ...
    proc bikeGeometry::create_GeometryFront {} {
            ...
            set help_03 [ vectormath::rotateLine       $Position(HandleBar)    100    [expr  90 - $Geometry(HeadTube_Angle) + $Geometry(Stem_Angle)]    ]
                #
            set Steerer(Stem)               [ vectormath::intersectPoint    $Position(HandleBar)  $help_03 $Steerer(Handlebar) $help_04 ]
                #
            ...
            set help_08  [ vectormath::addVector    $Position(BottomBracket_Ground) {200 0}]
                #
            set Position(Steerer_Ground)    [ vectormath::intersectPoint        $Steerer(Stem) $Steerer(Fork)      $Position(BottomBracket_Ground)  $help_08 ]
                #
            return
                #
    }
    ...
    proc bikeGeometry::create_GeometryCenter {} {
            ...
            set Position(SeatPost_Pivot)    [ vectormath::addVector $Position(SeatPost_Saddle)  [list 0 $SeatPost(PivotOffset)] -1]
                ...
                # set hlp_01              [ vectormath:::cathetusPoint {0 0} $Position(SeatPost_Saddle) [expr $SeatPost(Setback) - $SeatTube(OffsetBB)] {opposite}]
            ...
            set Geometry(SeatTube_Angle)    [ vectormath::angle $Position(SeatPost_SeatTube) $SeatTube(BottomBracket) [list -100 [lindex $SeatTube(BottomBracket) 1]]]
            set SeatTube(Direction)         [ vectormath::unifyVector $SeatTube(BottomBracket) $Position(SeatPost_SeatTube) ]
                #
            ...
            set help_08  [ vectormath::addVector    $Position(BottomBracket_Ground) {200 0}]
                #
            set Position(SeatTube_Ground)   [ vectormath::intersectPoint        $Position(SeatPost_SeatTube) $SeatTube(BottomBracket)      $Position(BottomBracket_Ground)  $help_08 ]
                #
            set Position(SeatTube_Saddle)   [ vectormath::intersectPoint [list 0 [lindex $Position(Saddle) 1] ] [list 100 [lindex $Position(Saddle) 1]] $SeatTube(BottomBracket) $Position(SeatPost_SeatTube) ]
                #
            ...
            # set Result(Position/Saddle)                  $Position(Saddle)
            # set Result(Position/SaddleNose)              $Position(SaddleNose)
            # set Result(Position/SaddleProposal)          $Position(SaddleProposal)
            # set Result(Position/SeatPostPivot)           $Position(SeatPost_Pivot)
            # set Result(Position/SeatPostSaddle)          $Position(SeatPost_Saddle)
            # set Result(Position/SeatPostSeatTube)        $Position(SeatPost_SeatTube)
            # set Result(Position/SeatTubeGround)          $Position(SeatTube_Ground)       ;# Point on the Ground in direction of SeatTube
            # set Result(Position/SeatTubeSaddle)          $Position(SeatTube_Saddle)
            # set SeatTube(Direction)         [list $Position(SeatTube_Ground)  $Position(SeatPost_SeatTube)]                
            # set Result(Tubes/SeatTube/Direction)         $SeatTube(Direction)                
            # set Result(Tubes/SeatTube/Direction)         [list $Position(SeatTube_Ground)  $Position(SeatPost_SeatTube)]                
                #
            return
                #
    }
    ...
    proc bikeGeometry::create_Reference {} {
            ...
            # set Result(Position/Reference_HB)          $Position(Reference_HB)
                #
            set Position(Reference_SN)   [list $SN_Distance $SN_Height]
            # set Result(Position/Reference_SN)          $Position(Reference_SN)
                #
    }    
    ...
    proc bikeGeometry::fill_resultValues {} {
            ...
            set position    $SeatTube(TopTube)
                ...
                # --- SeatTube/Angle ------------------------------
                #
            set angle [ vectormath::angle $Position(SeatPost_SeatTube) $SeatTube(BottomBracket) [list -200 [lindex $SeatTube(BottomBracket) 1]] ]
            ...
            set Geometry(SeatTube_TubeHeight)   $value
                ...
                # --- VirtualTopTube ----------------------------------
                #
            set Position(SeatTube_VirtualTopTube)   [ vectormath::intersectPoint [list -500 [lindex $HeadTube(Stem) 1]]  $HeadTube(Stem)  $SeatTube(BottomBracket) $Position(SeatPost_SeatTube) ]
                #
                # --- TopTube/VirtualLength -----------------------
                #
            set value       [expr [lindex $HeadTube(Stem) 0] - [lindex $Position(SeatTube_VirtualTopTube) 0] ]
            ...
            set Geometry(FrontWheel_x)      [ format "%.3f" $Geometry(FrontWheel_x) ]
                ...
                #
            #set position    $Position(RearWheel)
