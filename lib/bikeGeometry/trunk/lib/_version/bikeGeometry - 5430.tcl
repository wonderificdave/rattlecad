 ...
 #
    ...
    proc bikeGeometry::set_Config {key value} {
        ...
        puts "              <I> bikeGeometry::set_Config ... $object $key -> $returnValue"
            #
        return $configValue
            #
    }  
    ...
    proc bikeGeometry::get_Polygon {object {centerPoint {0 0}}} {
            ...
            switch -exact $object {
                ...
                Stem                        {set polygon $Polygon(Stem)             }
                HeadSetTop                  {set polygon $Polygon(HeadSetTop)       }
                HeadSetBottom               {set polygon $Polygon(HeadSetBottom)    }
                RearFender                  {set polygon $Polygon(RearFender)       }
                FrontFender                 {set polygon $Polygon(FrontFender)      }
                SeatPost                    {set polygon $Polygon(SeatPost)         }
                                                                    
                ChainStay                   {set polygon $Polygon(ChainStay)        }
                ChainStayRearMockup         {set polygon $Polygon(ChainStayRearMockup) }
                DownTube                    {set polygon $Polygon(DownTube)         }
                ForkBlade                   {set polygon $Polygon(ForkBlade)        }
                HeadTube                    {set polygon $Polygon(HeadTube)         }
                SeatStay                    {set polygon $Polygon(SeatStay)         }
                SeatTube                    {set polygon $Polygon(SeatTube)         }
                Steerer                     {set polygon $Polygon(Steerer)          }
                TopTube                     {set polygon $Polygon(TopTube)          }
