 ...
 #
    ...
    proc bikeGeometry::fill_resultValues {} {
            ...
            #set ChainSt_SeatSt_IS       [ bikeGeometry::get_Object     ChainStay/SeatStay_IS   position ]
                #  
            set Result(Angle_HeadTubeTopTube)             [format "%.4f" [ get_resultAngle $TopTube(HeadTube)     $Steerer(Stem)      $TopTube(SeatTube)  ]]
              # set Result(Angle/HeadTube/TopTube)        [ get_resultAngle $TopTube(HeadTube)     $Steerer(Stem)      $TopTube(SeatTube)  ]
              # puts "<I>   ... \$Result(Angle_HeadTubeTopTube) $Result(Angle_HeadTubeTopTube)"
            set Result(Angle_HeadTubeDownTube)            [format "%.4f" [ get_resultAngle $DownTube(HeadTube)    $BB_Position        $Steerer(Fork)      ]]
            set Result(Angle_SeatTubeTopTube)             [format "%.4f" [ get_resultAngle $TopTube(SeatTube)     $BB_Position        $TopTube(HeadTube)  ]]
            set Result(Angle_SeatTubeSeatStay)            [format "%.4f" [ get_resultAngle $SeatStay(SeatTube)    $ChainSt_SeatSt_IS  $BB_Position        ]]
            set Result(Angle_BottomBracketDownTube)       [format "%.4f" [ get_resultAngle $BB_Position           $DownTube(HeadTube) $TopTube(SeatTube)  ]]
            set Result(Angle_BottomBracketChainStay)      [format "%.4f" [ get_resultAngle $BB_Position           $TopTube(SeatTube)  $ChainSt_SeatSt_IS  ]]
            set Result(Angle_SeatStayChainStay)           [format "%.4f" [ get_resultAngle $ChainSt_SeatSt_IS     $BB_Position        $SeatStay(SeatTube) ]]                
