 ...
 #
    ...
    proc bikeGeometry::set_Config {key value} {
        ...
        puts "              <I> bikeGeometry::set_Config ... $key -> $returnValue"
            #
        return $configValue
            #
    }  
    ...
    proc bikeGeometry::get_Position {key {centerPoint {0 0}}} {
            ...
            switch -glob $key {
                ...
                FrontBrake_Shoe                     {set position $FrontBrake(Shoe)         }                
                FrontBrake_Help                     {set position $FrontBrake(Help)         }
                FrontBrake_Definition               {set position $FrontBrake(Definition)   }
                FrontBrake_Mount                    {set position $FrontBrake(Mount)        }
                RearBrake_Shoe                      {set position $RearBrake(Shoe)          }
                RearBrake_Help                      {set position $RearBrake(Help)          }
                RearBrake_Definition                {set position $RearBrake(Definition)    }
                RearBrake_Mount                     {set position $RearBrake(Mount)         }
                 
                CarrierMount_Front                  {set position $FrontCarrier(Mount)      }
                CarrierMount_Rear                   {set position $RearCarrier(Mount)       }
                DerailleurMount_Front               {set position $FrontDerailleur(Mount)   }
                ...
                Saddle_Proposal                     {set position $Saddle(Proposal)         }
                SeatPost_Pivot                      {set position $SeatPost(PivotPosition)  }
                SeatPost_Saddle                     {set position $SeatPost(Saddle)         }
                SeatPost_SeatTube                   {set position $SeatPost(SeatTube)       }
                SeatTube_Ground                     {set position $SeatTube(Ground)         }
                SeatTube_Saddle                     {set position $Position(SeatTubeSaddle) }
                SeatTube_VirtualTopTube             {set position $SeatTube(VirtualTopTube) }
                Steerer_Ground                      {set position $Steerer(Ground)          }
                ...
                SeatTube_BottleCage_Base            {set position $SeatTube(BottleCage_Base)            }
                SeatTube_BottleCage_Offset          {set position $SeatTube(BottleCage_Offset)          }
