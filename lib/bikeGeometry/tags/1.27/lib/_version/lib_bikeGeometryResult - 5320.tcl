 ...
 #
    ...
    proc bikeGeometry::set_Result_SeatTubeDirection     {_array _name value} {
            ...
            set height_Setback      [expr $SeatPost(Setback) * cos([vectormath::rad $value])]
                #
            set ST_height           [expr $Geometry(Saddle_Height) - $Saddle(SaddleHeight) - $SeatPost(PivotOffset) + $height_Setback]
    ...
    proc bikeGeometry::set_Result_FrontWheeldiagonal    {_array _name value} {
            ...
            set vect_02     [ expr $vect_01 - $Fork(Rake) ]
                #
            set dist_x      [ expr { sqrt( $value * $value - $Geometry(FrontWheel_Distance_y) * $Geometry(FrontWheel_Distance_y) ) } ]
            set position    [ list $dist_x $Geometry(FrontWheel_Distance_y)]
                #
