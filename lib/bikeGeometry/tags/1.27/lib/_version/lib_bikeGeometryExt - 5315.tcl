 ...
 #
    ...
    proc bikeGeometry::create_GeometryFront {} {
            ...
            set Steerer(Ground)         [ vectormath::intersectPoint        $Steerer(Stem) $Steerer(Fork)      $BottomBracket(Ground)  $help_08 ]
                #
            set Fork(Dropout)           [list $Geometry(FrontWheel_DistanceBB_x)    [expr $Geometry(BottomBracket_Depth) + ($Geometry(FrontWheel_Radius) - $Geometry(RearWheel_Radius))]]
                #
            return
                #
    }
    proc bikeGeometry::create_GeometryRear {} {
            ...
            variable Result
                #
            set RearWheel(Position)     [ list [expr -1.0 * $Geometry(RearWheel_DistanceBB_x)] $Geometry(BottomBracket_Depth) ]
                #
    ...
    proc bikeGeometry::create_RearWheel {} {
        ...
        set Geometry(RearWheel_DistanceBB_x)    [ expr sqrt(pow($Geometry(RearWheel_DistanceBB),2)  - pow($Geometry(BottomBracket_Depth),2)) ]
            #
        return    
            #
    }
