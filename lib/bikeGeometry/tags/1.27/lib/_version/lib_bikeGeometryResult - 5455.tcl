 ...
 #
    ...
    proc bikeGeometry::set_Result_SaddleSeatTube_BB     {value} {
            ...
            set pos_SeatTube_y          [expr $value * sin([vectormath::rad $Geometry(SeatTube_Angle)])]
                #
            set delta_Saddle_ST         [expr [lindex $Position(Saddle) 0] - [lindex $Position(SeatTubeSaddle) 0]]
                #
    ...
    proc bikeGeometry::set_Result_RearWheelRadius       {value} {
            ...
            variable RearWheel
                #
            puts "    <1> set_Result_RearWheelRadius   ... check $Geometry(RearTyre_Height)  ->  $value"
                #
            set Geometry(RearTyre_Height)   [ expr $value - 0.5 * $Geometry(RearRim_Diameter)]
                #
            bikeGeometry::update_Project
                #
            puts "    <2> set_Result_RearWheelRadius   ... check $Geometry(RearTyre_Height)  ->  $value"
                #
            return $Geometry(RearTyre_Height) 
                #
    }
    ...
    proc bikeGeometry::set_Result_FrontWheelRadius      {value} {
            ...
            variable FrontWheel
                #
            puts "    <1> set_Result_FrontWheelRadius   ... check $Geometry(FrontTyre_Height)  ->  $value"
                #
            set Geometry(FrontTyre_Height)  [ expr $value - 0.5 * $FrontWheel(RimDiameter)]
                #
            bikeGeometry::update_Project
                #
            puts "    <2> set_Result_FrontWheelRadius   ... check $Geometry(FrontTyre_Height)  ->  $value"
                #
