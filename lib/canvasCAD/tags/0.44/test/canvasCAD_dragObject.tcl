...
##+######################
	...
	namespace eval sketchboard {
      ...
      set dragObj_1 [$myCanvas  create   circle  	{160 60}   -radius 50 -tags {Line_01}  -fill blue   -width 2]
      #bind $dragObj_1 <3>         [list position.canvas.item [$myCanvas getPath] $dragObj_1 %X %Y]
      #bind $dragObj_1 <B1-Motion> [list drag.canvas.item     [$myCanvas getPath] $dragObj_1 %X %Y]
      
      bind $dragObj_1 <1>         [list canvasCAD::dragStart     %X %Y]
      bind $dragObj_1 <B1-Motion> [list canvasCAD::dragContent   [$myCanvas getPath] $dragObj_1 %X %Y]
      
      
      set dragObj [button [$myCanvas getPath].b -text "Test Button"]
      set id [[$myCanvas getPath] create window 100 100 -window $dragObj]

      # bind $dragObj <B3-Motion> [list [namespace current]::drag.canvas.item [$myCanvas getPath] $id %X %Y]
      bind $dragObj <3>         [list canvasCAD::dragStart     %X %Y]
      bind $dragObj <B3-Motion> [list canvasCAD::dragContent   [$myCanvas getPath] $id %X %Y]
      
      return $myCanvas
    }
      
		proc createStage_ {cv_path cv_width cv_height st_width st_height unit st_scale args} {
    ...
    proc canvas.canvas.item {canWin objName x y} {
        ...
        set drag_obj [$cv_obj create   oval  	{2 2 28 28} -fill blue  -width 2]
        $canWin create window $x $y  -anchor w  -window $cv_obj
        bind $cv_obj <3>         [list [namespace current]::position.canvas.item $cv_obj $drag_obj %X %Y]
        #bind $cv_obj <B1-Motion> [list [namespace current]::drag.canvas.item     $canWin $cv_obj %X %Y]
        
        return $cv_obj
    }
    
    proc position.canvas.item {canWin item newX newY} {
          set ::x $newX
          set ::y $newY
          puts "   -> $::x / $::y"
    }
