 ...
 namespace eval rattleCAD::configPanel {
    ...
    proc init_configValues {} {
			...
			foreach arrayNameComplete [appUtil::_childArrays [namespace current]] {
				...
				foreach key [lsort [array names $arrayNameComplete]] {
                    ...
                    puts "    .... [set  $arrayNameComplete\($key\)]"
                }
            }
				...
				foreach key [lsort [array names $arrayNameComplete]] {
                    ...
                    if {1 == 2} {
                            switch -exact $arrayName {
                                Component {
                                        switch -glob $key {
                                                *File {     set object [lindex [split $key /] 0]
                                                            puts "     ...     get_Component  .. $arrayName\($key\)     -> [rattleCAD::model::get_Component $object]" }
                                                default {   puts "     ...     get_Scalar     .. $arrayName\($key\)     -> [rattleCAD::model::get_Scalar  $arrayName $key]" }
                                        }
                                    }
                                default {
                                        puts "     ...     get_Scalar     .. $arrayName\($key\)     -> [rattleCAD::model::get_Scalar  $arrayName $key]"
                                    }
                            }
                    }
                        # puts "        -> $key   -> $value"
    ...
    proc updateCanvas {targetCanvas} {
			...
			switch -glob $compFile {
                etc:*  {  set fileName  [file join $::APPL_Config(::APPL_Config(COMPONENT_Dir)) [lindex [split $compFile {:}] 1] ] }
