 ...
 namespace eval rattleCAD::test {
    ...
    proc method_rattleCAD_HandleBarandSaddle {} {
        ...
        set step_TT_Angle     [expr  15.0  / $loopSteps]
            ...
            #
        set title       "  ... the rattleCAD-Method: "
        ...
        if 0 {       
                ...
                set demo_SD_Height    $init_SD_Height
                    #
                set step_SD_Distance  [expr  30.0  / $loopSteps]
                set step_SD_Height    [expr  50.0  / $loopSteps]
                    #
        ...
        createDemoText  $targetCanvas  "$title"
        rattleCAD::control::setValue {Scalar/Geometry/Stem_Angle  0} {update} noHistory         
        after 1000
            #
        if 1 {
                set loopCount      0
                set stepCount      $loopSteps
                    #
                # puts "[rattleCAD::model::set_Scalar     Geometry Stem_Angle  0]  " 
                    
                    
                set init_HB_Reach       [rattleCAD::model::get_Scalar     Geometry HandleBar_Distance]
                set init_HB_Stack       [rattleCAD::model::get_Scalar     Geometry HandleBar_Height]  
                set init_Stem_Length    [rattleCAD::model::get_Scalar     Geometry Stem_Length]
                    # set init_Reach_Length   [rattleCAD::model::get_Scalar     Geometry Reach_Length]
                    # set init_TT_Length      [rattleCAD::model::get_Scalar     Geometry TopTube_Virtual]   
                    #
                set demo_HB_Reach       $init_HB_Reach   
                set demo_HB_Stack       $init_HB_Stack
                set demo_Stem_Length    $init_Stem_Length
                    # set demo_Reach_Length $init_Reach_Length
                    # set demo_TT_Length    $init_TT_Length
                    #
                set _headAngle          [rattleCAD::model::get_Scalar     Geometry HeadTube_Angle]
                set __st_angle          [rattleCAD::model::get_Scalar     Geometry Stem_Angle]
                set _stemAngle          [expr 90 - $_headAngle + $__st_angle]
                set _compAngle          [expr 90 - $_headAngle - $__st_angle]
                    #
puts "   ... \$_stemAngle $_stemAngle"                    
                    #
                set step_HB_Reach     [expr  36.0  / $loopSteps]
                set step_HB_Stack     [expr  24.0  / $loopSteps]
                    # set step_Reach_Length 0
                    # set step_Stem_Length  [expr  30.0  / $loopSteps]
                    # set step_TT_Length    0
                    # 
                puts "         -> \$init_HB_Reach      $init_HB_Reach / $step_HB_Reach"
                puts "         -> \$init_HB_Stack      $init_HB_Stack / $step_HB_Stack"
                puts "         -> \$init_Stem_Length   $init_Stem_Length ... $_stemAngle"
                    # puts "         -> \$init_Reach_Length  $init_Reach_Length /$step_Reach_Length"
                    # puts "         -> \$init_Stem_Length  $init_Stem_Length /$step_Stem_Length"
                    # puts "         -> \$init_TT_Length    $init_TT_Length / $step_TT_Length"
                    #
                while {$loopCount < [expr $loopSteps * $maxLoops]} {            
                        # -- check \$runningStatus
                    if {![keepRunning]} {stopDemo cleanup; return}
                    ...
                    set step_Stem_Reach   [expr ($demo_HB_Reach - $init_HB_Reach) / sin([vectormath::rad $_headAngle])]
                        #
                    #set step_Stem_Stack   [expr ($demo_HB_Stack - $init_HB_Stack) * sin([vectormath::rad $_headAngle]) * cos([vectormath::rad $_headAngle]) / cos([vectormath::rad $_stemAngle]) ]
                    #set step_Stem_Reach   [expr ($demo_HB_Reach - $init_HB_Reach) / cos([vectormath::rad $_stemAngle]) * ( 1 - tan([vectormath::rad $_stemAngle]) * tan([vectormath::rad $_compAngle]) )]
                        #
                        # set demo_Stem_Length  [expr $init_Stem_Length + $step_Stem_Stack]
                    #set demo_Stem_Length  [expr $init_Stem_Length + $step_Stem_Stack ]
                    #set demo_Stem_Length  [expr $init_Stem_Length + $step_Stem_Reach]
                    set demo_Stem_Length  [expr $init_Stem_Length + $step_Stem_Stack + $step_Stem_Reach]
                    
                    
                    puts "->   $init_Stem_Length  $_headAngle  ... [expr ($demo_HB_Stack - $init_HB_Stack)] $step_Stem_Stack"
                    puts "->   $init_Stem_Length  $_headAngle  ... [expr ($demo_HB_Reach - $init_HB_Reach)] $step_Stem_Reach"
                    
                        # set demo_Stem_Length  [expr $init_Stem_Length + $step_Stem_Reach - $step_Stem_Stack]
                        # set demo_Reach_Length $init_Reach_Length
                        # set demo_TT_Length   $init_TT_Length
                    ...
                    lappend myList Scalar/Geometry/Stem_Length          $demo_Stem_Length
                        # lappend myList Scalar/Geometry/Reach_Length         $demo_Reach_Length
                        # lappend myList Scalar/Geometry/TopTube_Virtual      $demo_TT_Length
                        #
