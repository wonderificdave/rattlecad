...
# 
    ...
    namespace eval rattleCAD::control {
        ...
        variable  window_Size    {0}
          # ----------------- #
        variable _lastValue
        variable _editValue; array set _editValue {}
          # ----------------- #
        variable  Session              
        array set Session {
                    rattleCADVersion  {}
	...
	proc rattleCAD::control::updateControl {} {
        ...
        rattleCAD::control::get_editValues
            #
    ...
    proc rattleCAD::control::setValue {xpathValueList {mode {update}} {history {append}}} {
        ...
        foreach {xpath value} $xpathValueList {
                ...
                puts ""
                puts "    == START == rattleCAD::control::setValue =====================loop=="
                puts "          ... $mode/$history"
                    #
                # set oldValue [lindex [array get _lastValue $xpath] 1]
                set oldValue [lindex [array get _editValue $xpath] 1]
                    # puts "  -> $oldValue"
                    #
                ...
                puts ""
                puts "    == END ==== rattleCAD::control::setValue =====================loop=="
                puts ""
        }
            #
            # update View
    ...
    proc rattleCAD::control::get_editValues {} {
            #
    ...
    proc rattleCAD::control::dict_to_editValue {dictVar dictPath} {
            #
        dict for {key val} ${dictVar} {
            set path "$dictPath/$key"
            if {[catch {[namespace current]::dict_to_editValue  ${val} $path} eID]} {
                    # puts "       [format {%-10s ... %-40s ... %s} $rootNode $path $val]"
                array set [namespace current]::_editValue [list $path $val]
            }
        }
            #
        return
