#!/usr/bin/wish
 ...
 #
    ...
    #set currentDir         [file normalize [file dirname argv0] ]
    #puts "   -> \$currentDir $currentDir"
    #puts "   -> [file dirname $currentDir]"
    #puts "   -> [file join [file dirname $currentDir] lib]"
    #lappend auto_path      [file join [file dirname $currentDir] lib]
    ...
    # set      fileName      \"[file normalize [file join [file dirname argv0] __debug_3.4.01.60_road.xml]]\"
    puts "\n   ... $fileName\n"
    # exit
    ...
    proc insertSkeleton {tubeName position direction profile_xy profile_xz} {
            ...
            foreach node $tubeNodes_Regular {
                ...
                $targetText insert end "#        --> $project_NodeCount\n"
    ...
    proc insertWheel {location} {
            ...
            $targetText insert end "#\n# Wheel - $location \n#\n"
            $targetText insert end "#\n#  -> \$project_NodeCount - last: $project_NodeCount \n#\n"
              #
            $targetText insert end "#\ng  Wheel_Tyre_$location\n#\n"
              #
              # --- polygon_xy -- opposite ----------
              #
            $targetText insert end "# Wheel - $location  -- polygon_xz \n# ------------------\n"
              #
            set project_LastCount $project_NodeCount
              #
            for {set alpha 0} {$alpha<360} {incr alpha 5} {
                set x [expr $wheelRadius * cos([vectormath::rad $alpha]) + $pos_x]
                set y 0.00
                set z [expr $wheelRadius * sin([vectormath::rad $alpha]) + $pos_z]
                lappend nodeList [list $x $y $z]
                incr project_NodeCount
                $targetText insert end "#        --> $project_NodeCount\n"
                $targetText insert end "v $x $y $z\n"              
            ...
            $targetText insert end "#\n$shapeNode\n#\n"   
              ...
              # --- TyreShoulder ----------
              #
            $targetText insert end "#\n# Wheel - $location  -- TyreShoulder \n# ------------------\n"
            $targetText insert end "#\n#  -> \$project_NodeCount - last: $project_NodeCount \n#\n"
              #
            $targetText insert end "#\ng  Wheel_TyreShoulder_$location\n#\n"
              #
            set project_LastCount [expr $project_NodeCount +1]
            #set project_LastCount $project_NodeCount
            set nodeList {}
              #
            for {set alpha $startAngle} {$alpha<=[expr 360+$startAngle]} {incr alpha 5} {
                set x [expr $shoulderRadius * cos([vectormath::rad $alpha]) + $pos_x]
                set y1 [expr  0.5 * $width_TyreShoulder]
                set y2 [expr -0.5 * $width_TyreShoulder]
                set z [expr $shoulderRadius * sin([vectormath::rad $alpha]) + $pos_z]
                lappend nodeList [list $x $y1 $z]
                lappend nodeList [list $x $y2 $z]
                incr project_NodeCount
                $targetText insert end "#        --> $project_NodeCount\n"
                $targetText insert end "v $x $y1 $z\n"
                incr project_NodeCount
                $targetText insert end "#        --> $project_NodeCount\n"
                $targetText insert end "v $x $y2 $z\n"         
            }   
            puts "  - here"
            puts "  - here [llength $nodeList]"
            puts "  - \$project_NodeCount $project_NodeCount"
            puts "  - \$project_LastCount $project_LastCount"
            puts "  - [expr $project_NodeCount - $project_LastCount]"
              #
            #exit
              #
            set i_end_1  $project_LastCount
            set i_end_2  [expr $project_LastCount + 1]
            puts "   -> \$i_end_1 $i_end_1"
            puts "   -> \$i_end_2 $i_end_2"
            #exit
              #
            for {set i 0} {$i<[llength $nodeList]} {incr i 2} {
                #puts "    $i -> [lindex $nodeList $i] [lindex $nodeList $i+1]"
                #puts "    $i -> [lindex $nodeList $i+2] [lindex $nodeList $i+3]"
                set a  [expr $project_LastCount + $i]
                set b  [expr $project_LastCount + $i+1]
                set c  [expr $project_LastCount + $i+3]
                set d  [expr $project_LastCount + $i+2]
                puts "f $a $b $c $d"
                $targetText insert end "f $a $b $c $d\n"
            }
            puts "f $c $d $i_end_2 $i_end_1"
            $targetText insert end "f $c $d $i_end_2 $i_end_1\n"
    }
